  ...
  private fun disconnect(result: Result) {
    Thread {
      try {
        ...
        Handler(Looper.getMainLooper()).post {
          result.error("disconnect_failed", "disconnect error", null)
        }
      }
    }.start()
  }
/*
  private fun connect(result: Result, deviceId: String, serviceUuid: String) {
    try {
      ...
      assert(device != null)
      socket = device?.createInsecureRfcommSocketToServiceRecord(UUID.fromString(serviceUuid))
 ...
 */
  ...
  private fun connect(result: Result, deviceId: String, serviceUuid: String) {
    ...
    // Connect işlemini ayrı bir thread'de yap, main thread'i bloklamasın
    Thread {
      try {
        device = ba.getRemoteDevice(deviceId)
        android.util.Log.i("Bluetooth Connection", "device found")
        ...
        // Result.success'i main thread'den çağır (Flutter MethodChannel kuralı)
        Handler(Looper.getMainLooper()).post {
          result.success(true)
          publishBluetoothStatus(2)
        }
